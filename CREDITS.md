# Credits

KiauhTC is a fork of KIAUH with built-in support for Klipper toolchanger components.

---

## KIAUH — Klipper Installation And Update Helper

- **Author:** Dominik Willner (th33xitus)
- **Source:** https://github.com/dw-0/kiauh
- **License:** GNU General Public License v3

KiauhTC is a direct fork of KIAUH. All original KIAUH code retains its copyright
and license. Changes and additions are Copyright (C) 2026 Broncosis and are also
distributed under GNU GPLv3.

---

## klipper-toolchanger-easy

- **Author:** jwellman80
- **Source:** https://github.com/jwellman80/klipper-toolchanger-easy
- **License:** GNU General Public License v3

Installed on demand by the Toolchanger Kit extension. No source code is bundled;
it is cloned from upstream at install time.

---

## Axiscope

- **Author:** nic335
- **Source:** https://github.com/nic335/Axiscope
- **License:** MIT

Installed on demand by the Toolchanger Kit extension. No source code is bundled;
it is cloned from upstream at install time.

---

## Cartographer Klipper Plugin

- **Author:** Cartographer3D
- **Source:** https://github.com/Cartographer3D/cartographer-klipper
- **License:** GNU General Public License v3

Installed on demand by the Toolchanger Kit extension. No source code is bundled;
it is cloned from upstream at install time.

---

## Filament Feeder

- **Author:** Broncosis
- **Source:** https://github.com/broncosis/Filament_feeder
- **License:** GNU General Public License v3

Installed on demand by the Filament Feeder extension. No source code is bundled;
it is cloned from upstream at install time.

---

## Spoolman Lane Sync

- **Author:** Broncosis
- **Source:** https://github.com/broncosis/spoolman-lane-sync
- **License:** GNU General Public License v3

Installed on demand by the Spoolman Lane Sync extension. No source code is bundled;
it is cloned from upstream at install time.

---

## KlipperScreen Filament Lanes

- **Author:** Broncosis
- **Source:** https://github.com/broncosis/KlipperScreen-filament-lanes
- **License:** GNU General Public License v3

Installed on demand by the KlipperScreen Filament Lanes extension. No source code is bundled;
it is cloned from upstream at install time.

---

## v4l2-UI

- **Author:** nic335
- **Source:** https://github.com/nic335/v4l2-ui
- **License:** MIT

Installed on demand by the v4l2-UI extension. No source code is bundled;
it is cloned from upstream at install time.
